...
options = tf.compat.v1.python_io.TFRecordOptions(compression_type=tf.compat.v1.python_io.TFRecordCompressionType.GZIP)
...
def read_record(dataset, serialized_record):
    ...
    for value_name, value_type in dataset.values.items():
        ...
        if value_type == 'int':
            ...
            pass
    record = tf.io.parse_single_sequence_example(serialized=serialized_record, context_features=features, sequence_features=feature_lists)
    return record
...
def read_records(dataset):
    ...
    path_queue = tf.compat.v1.train.string_input_producer(string_tensor=paths, shuffle=True, capacity=len(paths))
...
def batch_records(dataset, batch_size, noise_range=None):
    ...
    if 'alternatives' in records:
        ...
        for value_name, record in sequence_records.items():
            ...
            records[value_name] = tf.compat.v1.Print(records[value_name], (sample, records[value_name]))
    batch = tf.compat.v1.train.shuffle_batch(tensors=records, batch_size=batch_size, capacity=(batch_size * 50), min_after_dequeue=(batch_size * 10), num_threads=1)
    ...
    for value_name, value_type in dataset.values.items():
        ...
        if noise_range is not None and noise_range > 0.0 and value_type == 'world':
            ...
            noise = tf.compat.v1.truncated_normal(shape=((batch_size,) + dataset.world_shape), mean=0.0, stddev=noise_range)
...
def write_records(dataset, records, path):
    ...
    with tf.compat.v1.python_io.TFRecordWriter(path=(path + '.tfrecords.gz'), options=options) as writer:
